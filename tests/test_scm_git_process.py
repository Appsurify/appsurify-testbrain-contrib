...
from testbrain.contrib.scm.git.process import GitProcess, GitVCS, GitSubmoduleProcess
from testbrain.contrib.scm.exceptions import ProcessError
from testbrain.contrib.scm.git.utils import parse_commits_from_text
...
def test_git_process_foreach_submodules_ls_files(fp):
    git_process = GitSubmoduleProcess()
    # git_process = GitProcess()
    # git_submodule_process = GitSubmoduleProcess()
    fp.register(
        ["git", "submodule", "foreach", "git", "ls-tree", "--name-only", "-r", "main"],
        stdout="""
        Entering 'deps/sub_test_repo'
        .gitignore
        LICENSE
...
def test_git_process_foreach_submodules_log(fp):
    git_process = GitSubmoduleProcess()
    ...
    fp.register(
        ...
        stdout=FAKE_LOG_SUBMODULE_PROCESS,
    )

    result = git_process.log(rev="HEAD", number=4)

    assert type(result) is str
    assert result == FAKE_LOG_SUBMODULE_PROCESS

    submodule_commits = parse_commits_from_text(result)

    assert len(submodule_commits) == 3
